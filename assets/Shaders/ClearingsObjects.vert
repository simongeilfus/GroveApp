...
void main(){
	...
	position.xyz *= 8.0;
	position.xyz += vec3( uPosition ) + vec3( 0, -1, 0 );
	vec3 center 	= ciTexCoord1.xyz;
	float height	= mix( texture( uHeightMapTemp, uPosition.xz ).r, texture( uHeightMap, uPosition.xz ).r, uHeightMapProgression ) * uElevation - 0.5;
